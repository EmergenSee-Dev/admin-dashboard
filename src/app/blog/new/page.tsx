...
const BlogNewForm = () => {
  // All state and hooks are now inside this component.
  ...
  useEffect(() => {
    ...
    const loadDraftOrBlog = async () => {
      ...
      if (savedData) {
        const { title, text, body, image } = JSON.parse(savedData);
        setTitle(title || '');
        setText(text || '');
        setBody(body || '');
        setImage(image || '');
  ...
  return (
    <main className='mx-4'>
      ...
      <button
        ...
        className='my-3 flex w-32 justify-evenly bg-white border rounded-full p-3'
      >
